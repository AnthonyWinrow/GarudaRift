...
void ADayCycle::BeginPlay()
{
	...
	if (PhaseColors.Num() > 0)
	{
		...
		Sunlight->LightColor = PhaseColors[CurrentPhaseIndex].ToFColor(true);
	...
	UE_LOG(LogDayCycle, Log, TEXT("BeginPlay: CurrentInensity (Before) =: CurrentIntensityIndex: %d, NextIntensityIndex: %d"), CurrentIntensityIndex, NextIntensityIndex);
